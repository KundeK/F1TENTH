...
public:
    Relay() : Node("relay") {
        subscription_ = this->create_subscription<ackermann_msgs::msg::AckermannDriveStamped>(
            "drive", 10,
            std::bind(&Relay::handle_message, this, std::placeholders::_1));
        publisher_ = this->create_publisher<ackermann_msgs::msg::AckermannDriveStamped>("drive_relay", 10);
    }

private:
    void handle_message(const ackermann_msgs::msg::AckermannDriveStamped::SharedPtr msg) {
        ...
        RCLCPP_INFO(this->get_logger(), "Publishing: '%f' '%f'", new_msg.drive.speed, new_msg.drive.steering_angle);
    }
